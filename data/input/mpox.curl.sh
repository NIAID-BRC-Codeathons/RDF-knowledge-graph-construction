...
# Example command to retrieve all fields for all the viral pathogens in TSV format from the ENA portal API.
curl -X POST -H "Content-Type: application/x-www-form-urlencoded" -d 'result=read_run&query=tag="pathogen:virus"&fields=ALL&format=tsv' "https://www.ebi.ac.uk/ena/portal/api/search" > ./mpox.tsv
